import { saturatedColors, shadedColors, mutedColors, tintedColors } from './colors.js';
...
let currentColors = saturatedColors; 
let allCategories = [];
...
function setupWheelSelector() {
    wheelTypeSelect.addEventListener('change', (e) => {
        switch(e.target.value) {
            case 'shaded':
                currentColors = shadedColors;
                break;
            case 'muted':
                currentColors = mutedColors;
                break;
            case 'tinted':
                currentColors = tintedColors;
                break;
            default:
                currentColors = saturatedColors;
        }
        updateCategoryFilters();
        renderColorWheel();
    });
...
function updateCategoryFilters() {
    ...
    allCategories = [...new Set(currentColors.flatMap(color => color.categories || []))];
...
function renderColorWheel() {
    ...
    const [validColors, invalidColors] = currentColors.reduce((acc, color) => {
